...
def get_fields(type_id):
    try:
        with get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT id, name, field_type, dropdown_values, dictionary_id FROM incident_field WHERE type_id = %s", (type_id,))
            fields = cur.fetchall()
        return jsonify([{"id": field[0], "name": field[1], "field_type": field[2], "dropdown_values": field[3], "dictionary_id": field[4]} for field in fields])
    except pg8000.dbapi.DatabaseError as e:
...
def create_incident():
    try:
        ...
        with get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT id, name, field_type FROM incident_field WHERE type_id = %s", (type_id,))
            fields = cur.fetchall()

        create_table_for_incident_type(type_id, fields)

        field_names = ', '.join([f[1] for f in fields])
        field_values = ', '.join([f'%({f[1]})s' for f in fields])
        insert_query = f"""
            INSERT INTO incident_type_{type_id} (organization_id, category_id, type_id, creator, responsible, evidence_link, evidence_name, status, {field_names})
            VALUES (%(organization_id)s, %(category_id)s, %(type_id)s, %(creator)s, %(responsible)s, %(evidence_link)s, %(evidence_name)s, %(status)s, {field_values})
            RETURNING id, created_at, updated_at
        """
        with get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute(insert_query, {**data, **custom_fields})
